...
with tab_scan:
    ...
    with col_b:
        ...
        if st.button("Capture test alert"):
            ...
            if st.session_state.get("enable_email_alerts", False):
                smtp_user = st.secrets.get("SMTP_USER", "")
                smtp_app_password = st.secrets.get("SMTP_APP_PASSWORD", "")
                to_address = st.secrets.get("ALERT_TO", smtp_user)

                if smtp_user and smtp_app_password and to_address:
                    subject, body = format_alert_email(test)
    ...
    if r.bias in ["LONG", "SHORT"] and r.setup_score >= alert_threshold:
        if can_alert(r.symbol, now, cooldown_minutes):
            ...
            if enable_email_alerts:
                subject, body = format_alert_email(row)
                ok, err = send_email_alert(
                    smtp_user=smtp_user,
                    smtp_app_password=smtp_app_password,
                    to_address=to_address,
